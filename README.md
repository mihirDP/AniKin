...
* <img src="AniKin/scripts/anikin/icons/cam_lock.svg" width="18"> **Cam-Lock to Object:** Lock the viewport camera to any selected object during playback. It uses a robust, constraint-based architecture (no scriptJobs). Choose between **Track Mode** (maintains exact offset) and **Aim Mode** (continuous framing) This mode highly recommended if you want to keep your Animated obj in focus while the Animation keeps playing. It safely restores the camera when unlocked without polluting the scene.
